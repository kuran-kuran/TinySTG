...
,frame_buffer_size(WIDTH * 2 * HEIGHT * 2)
{
	if(color_mode == 16)
	{
		this->buffer_size *= 2;
	...
	this->frame_buffer = new unsigned short[this->frame_buffer_size];
	memset(this->frame_buffer, 0, sizeof(unsigned short) * this->frame_buffer_size);
	LCD_1IN14_Display(this->frame_buffer);
}
	...
	if(this->color_mode == 8)
	{
		for(int y = 0; y < HEIGHT; ++ y)
		{
			for(int x = 0; x < WIDTH; ++ x)
			{
				...
				int xx = x * 2;
				int yy = y * 2;
				this->frame_buffer[yy * width2 + xx] = color;
				this->frame_buffer[yy * width2 + xx + 1] = color;
				this->frame_buffer[(yy + 1) * width2 + xx] = color;
				this->frame_buffer[(yy + 1) * width2 + xx + 1] = color;
			}
		}
	}
	else
